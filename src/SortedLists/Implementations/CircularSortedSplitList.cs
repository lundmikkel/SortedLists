namespace SortedLists
{
    ...
    public class CircularSortedSplitList<T> : ISortedList<T>
        where T : IComparable<T>
    {
        #region Fields

        private readonly List<CircularArray<T>> _lists;
        ...
        public CircularSortedSplitList(int deepness = 512)
        {
            ...
            _deepness = deepness;
            _lists = new List<CircularArray<T>> { new CircularArray<T>(_deepness) };
        ...
        public bool Add(T item)
        {
            ...
            else if (itemIndex == list.Count && listIndex == _lists.Count - 1)
            {
                ...
                list = new CircularArray<T>(_deepness);
                list.Insert(0, item);
                _lists.Add(list);
            ...
            else
            {
                ...
                if (listIndex == _lists.Count - 1 || mid <= _lists[listIndex + 1].Count)
                {
                    ...
                    _lists.Insert(listIndex + 1, new CircularArray<T>(_deepness));
                ...
                _lists[listIndex + 1].Move(list, mid, mid);
        ...
        {
            _lists.Clear();
            _lists.Add(new CircularArray<T>(_deepness));
        ...
        private static int GetItemIndex(CircularArray<T> list, T item)
        {
            var low = 0;
